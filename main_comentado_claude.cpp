  ...
  <body>
    ...
    <div class="modal-overlay" id="alertModal">
      <div class="modal-content">
        ...
        <p class="modal-message">
          El sistema ha estado en <strong>Modo Manual ON</strong> durante más de 5 minutos. 
      ...
      <div class="grid">
        ...
        <div class="card">
          ...
          <div class="metric-row">
            ...
            <span class="metric-row-value"><span id="luxTarget">800</span> lux</span>
          </div>
          <div class="metric-row">
    ...
    <script>
      ...
      function updateUI() {
        ...
        if (!systemData.autoMode && systemData.lightsOn && systemData.manualOnSeconds >= 300) {
        ...
        document.getElementById("luxTarget").textContent = systemData.luxTarget || 800;
      ...
      function updateModeUI() {
        ...
        if (autoMode) {btnAuto.classList.add("active");
...
// ============================================================================
constexpr unsigned long MANUAL_WARNING_THRESHOLD_SEC = 300;     // 5 minutos (300 segundos) para alerta
constexpr unsigned long DAILY_REPORT_INTERVAL_SEC = 86400;      // 24 horas para reporte diario
constexpr unsigned long STARTUP_DELAY_SEC = 60;                 // 1 minuto de espera antes del mensaje de inicio
constexpr unsigned long NOTIFICATION_MIN_INTERVAL_SEC = 60;     // Mínimo 1 minuto entre notificaciones
...
struct TelegramState {
    bool startupMessageSent = false;                // Flag para mensaje de inicio
    ...
    unsigned long lastNotificationTime = 0;         // Timestamp de última notificación (cualquier tipo)
...
void setup() {
    ...
    Serial.println(F("[INFO] Sistema inicializado. Verificando salud antes de notificar..."));
}
...
void loop() {
    ...
    // Verificaciones de notificaciones (cada 10 segundos para no saturar)
    static unsigned long lastNotificationCheck = 0;
    if (now - lastNotificationCheck >= 10000) {
        checkStartupMessage();      // Verifica mensaje de inicio
        checkManualModeWarning();   // Verifica alerta de modo manual
        checkDailyReport();         // Verifica reporte diario
        checkSystemHealth();        // Verifica errores del sistema
...
/**
 * Verifica la salud del sistema de forma continua
 */
void verifySystemHealth() {
    unsigned long now = millis();
    unsigned long uptimeSec = (now - state.systemStartTime) / 1000UL;
    
    // Verificar que han pasado al menos 5 segundos desde el inicio
    if (uptimeSec < 5) {
    ...
    // Verificar sensores (si se leyeron en los últimos 2 segundos)
    unsigned long timeSinceLastSensor = (now - state.lastSuccessfulSensorRead) / 1000UL;
    bool sensorsOk = (timeSinceLastSensor <= 2);
    
    // Sistema saludable si WiFi y sensores están OK
    if (wifiOk && sensorsOk) {
...
/**
 * Verifica si puede enviar una notificación (para evitar spam)
 */
bool canSendNotification() {
    unsigned long now = millis();
    unsigned long timeSinceLastNotification = (now - telegram.lastNotificationTime) / 1000UL;
    
    // Permitir notificación solo si han pasado al menos NOTIFICATION_MIN_INTERVAL_SEC
    return (timeSinceLastNotification >= NOTIFICATION_MIN_INTERVAL_SEC);
}

/**
 * Verifica y envía el mensaje de inicio del sistema (después de 1 minuto)
 */
void checkStartupMessage() {
    // Si ya se envió, no hacer nada
    if (telegram.startupMessageSent) {
        return;
    }
    ...
    // Verificar que han pasado al menos STARTUP_DELAY_SEC segundos
    if (uptimeSec < STARTUP_DELAY_SEC) {
        return;
    }
    
    // Verificar que el sistema esté saludable
    if (!state.systemHealthy) {
        Serial.println(F("[STARTUP] Sistema no saludable aún. Esperando..."));
        return;
    }
    
    // Verificar que se pueda enviar notificación
    if (!canSendNotification()) {
        return;
    }
    
    // Todo OK, enviar mensaje de inicio
    ...
    if (sendTelegramMessage(startMsg)) {
        telegram.startupMessageSent = true;
        telegram.lastNotificationTime = now;
        Serial.println(F("[STARTUP] ✓ Mensaje de inicio enviado correctamente"));
    }
}
...
/**
 * Verifica si debe enviar alerta de modo manual prolongado (5 minutos)
 */
void checkManualModeWarning() {
    unsigned long now = millis();
    ...
    // 3. Han pasado más de 5 minutos (300 segundos)
    // 4. No se ha enviado alerta en los últimos 10 minutos (para evitar spam)
    // 5. Se puede enviar notificación (intervalo mínimo)
    
    if (!config.autoMode && state.lightsOn) {
        unsigned long timeSinceManualOn = (now - state.manualOnStartTime) / 1000UL;
        
        if (timeSinceManualOn >= MANUAL_WARNING_THRESHOLD_SEC) {
            // Verificar si ya pasaron 10 minutos desde la última alerta
            unsigned long timeSinceLastWarning = (now - telegram.lastManualWarningTime) / 1000UL;
            
            if (!telegram.manualWarningAlreadySent || timeSinceLastWarning >= 600) {
                // Verificar que se pueda enviar notificación
                if (!canSendNotification()) {
                    return;
                }
                
                // Construir mensaje de alerta (sin la línea de "Responda 'AUTO'")
...
void checkSystemHealth() {
    ...
    if (WiFi.status() != WL_CONNECTED && state.wifiConnected) {
        ...
        // Verificar que se pueda enviar notificación
        if (!canSendNotification()) {
            state.wifiConnected = true; // Actualizar estado pero no notificar aún
            return;
        }
    ...
    if (timeSinceLastSensor > 10 && state.sensorsHealthy) {
        ...
        // Verificar que se pueda enviar notificación
        if (!canSendNotification()) {
            state.sensorsHealthy = true; // Actualizar estado pero no notificar aún
            return;
        }
    ...
    // Enviar notificación si hay error y es diferente al último
    if (errorDetected && errorMsg != telegram.lastErrorMessage) {
        ...
        if (timeSinceLastError >= 120) { // Mínimo 2 minutos entre errores
...
void checkDayRollover() {
    if (millis() - state.lastDayRollover >= 86400000UL) {
        ...
        if (canSendNotification()) {
            sendDailyReportTelegram();
            telegram.lastNotificationTime = millis();
        }
        ...
        [](AsyncWebServerRequest *req, uint8_t *data, size_t len, size_t, size_t) {
            ...
            if (deserializeJson(doc, data, len) == DeserializationError::Ok) {
                ...
                if (action == "on") {
                    ...
                    state.manualOnStartTime = now;
                    
                    // Notificación de cambio a manual ON (solo si se puede enviar)
                    if (canSendNotification()) {
                    ...
                    // Notificación de cambio a manual OFF (solo si se puede enviar)
                    if (canSendNotification()) {
                        String msg = "🎛️ *Modo Manual Activado*\n\n";
                        msg += "• Estado: *APAGADO*\n";
                        msg += "• Consumo: *0W*\n\n";
                    ...
                    // Notificación de cambio a automático (solo si se puede enviar)
                    if (canSendNotification()) {
